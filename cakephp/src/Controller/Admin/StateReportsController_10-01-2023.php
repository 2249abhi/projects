        ...
        //{
          ...
          //  echo $this->request->session()->read('Auth.User.state_code');
           ...
           // $dairies = $this->CooperativeRegistrations->find('list',['keyField'=>'district_code','valueField'=>'count']);  
            ...
            $dairies = $dairies->select(['district_code','count' => $dairies->func()->count('sector_of_operation')])->where(['sector_of_operation IN' => [9,37],'state_code'=>$this->request->session()->read('Auth.User.state_code'), 'is_draft'=>0, $condtion_district])->group(['district_code'])->toArray();

             //Dairies today 
            $dairies_today = $this->CooperativeRegistrations->find('list',['keyField'=>'state_code','valueField'=>'count']);
            
            $dairies_today = $dairies_today->select(['state_code','count' => $dairies_today->func()->count('sector_of_operation')])->where(['sector_of_operation IN' => [9,37],'is_draft'=>0,'DATE(created)' => date('Y-m-d'), $condtion_district])->group(['state_code'])->toArray();
            ...
            $pacs = $pacs->select(['district_code','count' => $pacs->func()->count('sector_of_operation')])->where(['sector_of_operation IN' => [1,59,20,22],'state_code'=>$this->request->session()->read('Auth.User.state_code') , 'is_draft'=>0, $condtion_district])->group(['district_code'])->toArray();


              //Pacs today
              $pacs_today = $this->CooperativeRegistrations->find('list',['keyField'=>'district_code','valueField'=>'count']);            
              $pacs_today = $pacs_today->select(['district_code','count' => $pacs_today->func()->count('sector_of_operation')])->where(['sector_of_operation IN' => [1,59,20,22],'state_code'=>$this->request->session()->read('Auth.User.state_code') , 'is_draft'=>0, 'DATE(created)' => date('Y-m-d'), $condtion_district])->group(['district_code'])->toArray();

            //Fisheries
            $fisheries = $this->CooperativeRegistrations->find('list',['keyField'=>'district_code','valueField'=>'count']);
            
            $fisheries = $fisheries->select(['district_code','count' => $fisheries->func()->count('sector_of_operation')])->where(['sector_of_operation IN' => [10,43],'state_code'=>$this->request->session()->read('Auth.User.state_code'),'is_draft'=>0, $condtion_district])->group(['district_code'])->toArray();

                //Fisheries today 
                $fisheries_today = $this->CooperativeRegistrations->find('list',['keyField'=>'district_code','valueField'=>'count']);
            
                $fisheries_today = $fisheries_today->select(['district_code','count' => $fisheries_today->func()->count('sector_of_operation')])->where(['sector_of_operation IN' => [10,43],'state_code'=>$this->request->session()->read('Auth.User.state_code'),'is_draft'=>0, 'DATE(created)' => date('Y-m-d'), $condtion_district])->group(['district_code'])->toArray();
