 ...
 */
class FunctionalChartController extends AppController
{
        ...
        if(!empty($s_sector))
        {
            if(empty($s_state))
            {
                ...
                if(empty($s_district))
                {
                    ...
                    $total_village_count = $total_village_count->select(['gram_panchayat_code','count' => $total_village_count->func()->count('gram_panchayat_code')])->where([$search_condition,$gp_search_condition])->group(['gram_panchayat_code'])->toArray();
