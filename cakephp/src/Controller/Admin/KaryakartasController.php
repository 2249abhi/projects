 ...
 */
class KaryakartasController extends AppController
{

    public function initialize()
    {
        parent::initialize();
        $this->loadModel('Karyakartas'); 
    }
    ...
    public function index()
    {
        ...
        if($this->request->is('get')){
            if(!empty($this->request->query['export_excel'])) {
                $queryExport = $this->Karyakartas->find('all',[
                    'conditions'=>[$searchString],
                    'limit' => $page_length,
                    'page'=> $page,
                    'order' => ['Karyakartas.id' => 'asc'],
                ]);
                $queryExport->hydrate(false);
                $ExportResultData = $queryExport->toArray();
                $fileName = "Karyakartas-".date("d-m-y:h:s").".xls";
        ...
        $rolesQuery = $this->Karyakartas->find('all', [
            'contain' => [],
            'order' => ['Karyakartas.id' => 'asc'],
            'conditions' => [$searchString]
        ]);
    ...
    public function add()
    {
        $role = $this->Karyakartas->newEntity();
        if ($this->request->is('post')) {
            $role = $this->Karyakartas->patchEntity($role, $this->request->getData());

            if(isset($this->request->data['image']['error']) && $this->request->data['image']['error']==0){
                $photo = $this->uploadFiles('karyakarta', $this->request->data['image']); 
                $role->image = $photo['filename'];
            }

            if ($this->Karyakartas->save($role)) {
                $this->Flash->success(__('The karyakarta has been saved.'));

                return $this->redirect(['action' => 'index']);
            }
            $this->Flash->error(__('The karyakarta could not be saved. Please, try again.'));
        }
    ...
    public function delete($id = null)
    {
        ...
        $role = $this->Karyakartas->get($id);
        if ($this->Karyakartas->delete($role)) {
            $this->Flash->success(__('The karyakarta has been deleted.'));
        } else {
            $this->Flash->error(__('The karyakarta could not be deleted. Please, try again.'));
        }
