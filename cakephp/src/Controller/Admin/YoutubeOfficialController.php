 ...
 */
class YoutubeOfficialController extends AppController
{
    public function initialize()
    {
        parent::initialize();
        $this->loadModel('YoutubeOfficial');
    }
    ...
    public function index()
    {
        ...
        if (!empty($data['title'])) {
            ...
            $this->set('title', $event_title);
            $search_condition[] = "YoutubeOfficial.title like '%" . $event_title . "%'";
        }
        if (isset($data['status']) && $data['status'] !='') {
            $status = trim($data['status']);
            $this->set('status', $status); 
            $search_condition[] = "YoutubeOfficial.status = '" . $status . "'";
        }
        ...
        if($this->request->is('get')){
            if(!empty($this->request->query['export_excel'])) {
                $queryExport = $this->YoutubeOfficial->find('all',[
                    'conditions'=>[$searchString],
                    'limit' => $page_length,
                    'page'=> $page,
                    'order' => ['YoutubeOfficial.id' => 'asc'],
                ]);
                $queryExport->hydrate(false);
                $ExportResultData = $queryExport->toArray();
                $fileName = "YoutubeOfficial-".date("d-m-y:h:s").".xls";
        ...
        $rolesQuery = $this->YoutubeOfficial->find('all', [
            'order' => ['YoutubeOfficial.id' => 'asc'],
            'conditions' => [$searchString]
        ]);
    ...
    public function add()
    {
        $doc_type = $this->YoutubeOfficial->newEntity();
        if ($this->request->is('post')) {
            $data       = $this->request->getData();
            $data       = $this->Sanitize->clean($data);
            
            $doc_type               = $this->YoutubeOfficial->patchEntity($doc_type, $data);
            ...
            $query = $this->YoutubeOfficial->query();
            $query->update()
                  ->set(['status' => 0])
                  ->execute();
            if ($this->YoutubeOfficial->save($doc_type)) {
                $this->Flash->success(__('The Youtube official has been saved.'));

                return $this->redirect(['action' => 'index']);
            }
            $this->Flash->error(__('The Youtube official could not be saved. Please, try again.'));
        }
    ...
    {
        $doc_type = $this->YoutubeOfficial->get($id);
        if ($this->request->is(['patch', 'post', 'put'])) {
            $data       = $this->Sanitize->clean($this->request->getData());
            
            $doc_type = $this->YoutubeOfficial->patchEntity($doc_type, $data);
            $doc_type->modified     = date('Y-m-d');
            /*$doc_type->title        = $data['title'];
            $doc_type->youtube_link = $data['youtube_link'];
            $doc_type->youtube_id   = $data['youtube_id'];
            $doc_type->publish_date = $data['publish_date'];
    ...
    public function delete($id = null)
    {
        ...
        $role = $this->YoutubeOfficial->get($id);
        ...
        $query = $this->YoutubeOfficial->query();
            ...
            ->execute(); 
        if ($query) {
            $this->Flash->success(__('The Youtube official has been deleted.'));
        } else {
            $this->Flash->error(__('The Youtube official could not be deleted. Please, try again.'));
        }
