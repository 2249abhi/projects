    ...
    public function index()
    {
      ...
      if($this->request->session()->read('Auth.User.role_id') == 11)
       ...
       $presentFunctionalStatus=$this->PresentFunctionalStatus->find('list',['keyField'=>'id','valueField'=>'name'])->where(['status'=>1])->order(['orderseq'=>'ASC'])->toArray();
         ...
         $headerRow = array("S.No", "Cooperative Society Name", "Location", "State", "District-Urban Local Body","Block name","gram_panchayat", "Sector", "Registration Number","Cooperative ID","Reference Year","Date of Registration","Status","Functional-Status");
         ...
         $data[] = [$i, $cooperative_society_name, $location,  $state_name, $dist_urbn ,$bloack_name,$panchayat_name, $sectors[$rows['sector_of_operation']],  $registration_number, $coperative_id, $rows['reference_year'], $rows['date_registration'],$astatus[$rows['is_approved']], $function_status]; 
         $i++;
         }
