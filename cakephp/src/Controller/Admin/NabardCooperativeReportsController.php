...
class NabardCooperativeReportsController extends AppController
{
    ...
    public function linacReport()
    {
       ...
       ini_set('memory_limit', '-1');
        ...
        if (isset($this->request->query['primary_activity']) && $this->request->query['primary_activity'] !='') {
            ...
            {
                $search_condition[] = "sector_of_operation_type = '" . $s_primary_activity . "'";
            }else{
                 $search_condition[] = "sector_of_operation = '" . $s_primary_activity . "'";
            }
        }
        else{
            $search_condition[] = "sector_of_operation_type = '" . 1 . "'";
            $this->set('s_primary_activity', 1);
        }
      ...
      $villagesarray = $this->DistrictsBlocksGpVillages->find('all')->where(['status'=>1])->order(['village_name'=>'ASC'])->toArray();
       ...
       $this->set('designations',$designations);
          ...
          { 
             ...
             ])->select(['id', 'cooperative_society_name', 'sector_of_operation','gram_panchayat_code','village_code','contact_person','designation','mobile','email'])->where(['is_draft'=>0,'status'=>1]);

             $queryExport->hydrate(false);
             $ExportResultData = $queryExport->toArray();
             $fileName = "LINAC_REPORT".date("d-m-y:h:s").".xls";
             $data = array(); 
           
             $headerRow = array("S.No", "Name of society",  "Sector", "Panchayat", "Village","ContactName","Designation",'Mobile','email');
           ...
           //   $this->strClean();
             ...
             $data[] = [$i,  $this->strClean(str_replace('  ', ' ', $rows['cooperative_society_name'])), $sectors[$rows['sector_of_operation']],   $dgbgV['panchayat'][$rows['gram_panchayat_code']], $dgbgV['village'][$rows['village_code']], trim($rows['contact_person']),  $this->strClean($designations[$rows['designation']]),$rows['mobile'], $rows['email']]; 
             $i++;
             }
            ...
            $registrationQuery = $this->CooperativeRegistrations->find('all', [
               ...
               // 'contain'=>['AreaOfOperationLevel','CooperativeRegistrationPacs'], 
        'conditions' => [$searchString,$search_condition2,$search_condition3]            
            ])->select(['id', 'cooperative_society_name', 'sector_of_operation','gram_panchayat_code','village_code','contact_person','designation','mobile','email'])->where(['CooperativeRegistrations.is_draft'=>0,'CooperativeRegistrations.status'=>1]);
         
           //CooperativeRegistrationsC      
  ...
  // $panchayatsarray = $this->DistrictsBlocksGpVillages->find('list',['keyField'=>'gram_panchayat_code','valueField'=>'gram_panchayat_name'])->where(['status'=>1])->group(['gram_panchayat_code'])->order(['gram_panchayat_name'=>'ASC'])->toArray();  
      ...
      //  $villagesarray = $this->DistrictsBlocksGpVillages->find('list',['keyField'=>'village_code','valueField'=>'village_name'])->where(['status'=>1])->order(['village_name'=>'ASC'])->toArray();
         ...
         $this->set('villagesarray',$villagesarray);
       ...
       ini_set('memory_limit', '-1');
        ...
        if($this->request->session()->read('Auth.User.role_id') == 8)
        {
            ...
            if(!empty($user_id))
            {
                ...
                $search_condition3['created_by IN'] = 0;
            }
