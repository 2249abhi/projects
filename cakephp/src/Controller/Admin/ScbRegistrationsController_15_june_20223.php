            ...
            //===============================================
             ...
             if($data['pacs_member'] == 0)
             {
               ...
               if(empty($data['pacs_is_member_affiliated']) && !empty($data['pacs_is_member_not_affiliated']))
               {
				...
				$tobe_remove_pacs =$this->CooperativeRegistrations->find('all')->where(['status'=>1,'operational_district_code IN'=>$societynmemberArray,'is_affiliated_union_federation'=>1,'sector_of_operation IN '=>[1,20,22]])->order(['operational_district_code'=>'ASC'])->toArray(); 
...
// Start TypesOfMembership for dropdown
    ...
    // End TypesOfMembership for dropdown
       ...
       $this->loadMOdel('LevelOfAffiliatedUnion'); 
		...
		//echo "<pre>" ; print_r($primary_activities11) ; exit ;
        $this->set(compact('StateCooperativeBank','states','CooperativeSocietyTypes','areaOfOperationsUrban','areaOfOperationsRural','PrimaryActivities','SecondaryActivities','PresentFunctionalStatus','pActivities','years','register_authorities','sectors','designations','water_body_type','federationlevel','sectorUrbans','area_of_operation_id_urban','area_of_operation_id_rural','all_districts','dccb_banks','society','society1','primary_activities','primary_activities11','typeofmembership11','dcb_bank_list'));
    }
    ...
    {    
	   $this->viewBuilder()->setLayout('ajax');
        if($this->request->is('ajax')){
            ...
            if($pacs_is_member_affiliated[0]!=''){
				//echo "<pre>" ; echo count($pacs_is_member_affiliated); print_r($pacs_is_member_affiliated) ;  
                $society_not_affiliated =$this->CooperativeRegistrations->find('all')->where(['status'=>1,'operational_district_code IN'=>$district_code,'id NOT IN'=>$pacs_is_member_affiliated,'sector_of_operation IN '=>[1,20,22]])->order(['operational_district_code'=>'ASC'])->toArray(); 
               }else{
				   $society_not_affiliated =$this->CooperativeRegistrations->find('all')->where(['status'=>1,'operational_district_code IN'=>$district_code,'sector_of_operation IN '=>[1,20,22]])->order(['operational_district_code'=>'ASC'])->toArray(); 
			   }
