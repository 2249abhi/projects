...
class ScbRegistrationsController extends AppController
{
    ...
    public function add()
    {
        ...
        $bank_name = $this->DistrictCentralCooperativeBank->find('all')->where(['entity_type' => 'stcb'])->first();
        ...
        if($this->request->is('post')){
            ...
            $StateCooperativeBank = $this->StateCooperativeBanks->patchEntity($StateCooperativeBank, $data,['associated' => ['ScbImplementingSchemes','ScbContactDetails']]);
    ...
    public function addMember()
    {
        ...
        $member_entered = $this->DcbScbMembers->find()->where(['created_by'=>$this->request->session()->read('Auth.User.id'),'status'=>1,'is_delete'=>0])->count();
   ...
   public function getdccb()
    ...
    public function memberList()
    {
        ...
        if($this->request->session()->read('Auth.User.role_id') == 24)
        {
            ...
            //DEO
            $bankExist = $this->StateCooperativeBanks->find()->where(['created_by'=>$this->request->session()->read('Auth.User.created_by'),'status'=>1,'is_delete'=>0])->first();
        }
        ...
        if (!empty($this->request->data['name'])) {
            ...
            $this->set('name', $name);
            $search_condition[] = "organization_name like '%" . $name . "%'";
        }
