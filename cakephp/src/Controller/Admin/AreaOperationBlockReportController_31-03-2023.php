             ...
             if(!empty($this->request->query['export_excel'])) { 
              ...
              //  $gram_panchayat_array= $this->DistrictsBlocksGpVillages->find('list',['conditions' => ['status'=>1,$condtiongp] ,'keyField'=>'gram_panchayat_code','valueField'=>'gram_panchayat_name'] )->group(['gram_panchayat_code'])->toarray();
               ...
               $not_covered_village=count($village_code_array)-$total_village_code;
                $header1 =array("No of Gram Panchayat", count($gram_panchayat_array),"Covered Gram Panchayat", $total_panchayat_code,"Not covered Gram Panchayat", $not_covered_gram_panchayat,"Number Of Village", count($village_code_array),"Covered Village", $total_village_code , "Not covered Village", $not_covered_village);
                ...
                $headerRow = array("S.No", "Panchayat", "Covered Village", "Total Covered Village", "Uncovered Village","Total Uncovered Village");
                    ...
                    if(count($covered_panchayatarray)>0)
                    {
                        foreach ($covered_panchayatarray as $key=>$pactive_value)
                        {
                            ...
                            $data[] = [$i, $pactive_value['panchayat_name'], $pactive_value['covered_village_name'], $pactive_value['count_covered_village'], $pactive_value['un_covered_village_name'], $pactive_value['count_uncovered_village']];
                            $i++;
                        }
                    }else{
                        $data[] = ['Nill',  '', '',  '', '','', '', ''];
                    ...
                    {
                        $data[] = [$k, $pactive_valuep['panchayat_name'], $pactive_valuep['covered_village_name'], $pactive_valuep['count_covered_village'], $pactive_valuep['un_covered_village_name'], $pactive_valuep['count_uncovered_village']];                       
                        $k++;
                    }
                    }else{
                        $data[] = ['Nill',  '', '',  '', '','', '', ''];
