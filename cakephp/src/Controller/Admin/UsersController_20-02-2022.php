...
class UsersController extends AppController
{
    ...
    public function index()
    {
        ...
        if($loginUser['role_id']==2){          
           $users = $this->paginate($this->Users->find('all')->where(['role_id IN'=>['7','8','11','13','15'], $searchString])->order(['Users.id'=>'DESC']));  
        } 
        ...
        if($loginUser['role_id']==2){          
            $roles=$this->Roles->find('list',['keyField'=>'id','valueField'=>'name'])->order(['name'=>'ASC'])->where(['id IN'=>[7,8,11,13,15]])->toArray(); 
        }
		else if($loginUser['role_id']==21){          
    ...
    public function edit($id = null)
    {
        ...
        $pactivities=$this->PrimaryActivities->find('list',['keyField'=>'id','valueField'=>'name'])->order(['name'=>'ASC'])->where(['status'=>1]);
        $this->set(compact('user', 'roles', 'states','districts', 'loginUser', 'pactivities','nationalList','role_id'));
    }
