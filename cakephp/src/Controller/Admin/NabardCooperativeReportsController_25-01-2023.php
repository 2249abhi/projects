        ...
        $this->set('sOption',$stateOption);
          ...
          { 
            ...
            $designations=$this->Designations->find('list',['keyField'=>'id','valueField'=>'name'])->order(['name'=>'ASC'])->where(['status'=>1])->toArray();
             ...
             $headerRow = array("S.No", "Name of the PACS/LAPS/FSS/Others(M)", "Registration Number of PACS", "Year of Registration", "Category-PACS/LAMPS/FSS/Other(M)", "Panchayat", "Village","Name of the Bank to which the PACS is Affilited","Total No.of members","No Of Villages Covered By Pacs","Office  Building","status","ContactName","Designation",'Mobile');
           ...
           //   $this->strClean();
             ...
             $data[] = [$i, trim($rows['cooperative_society_name']), trim($rows['registration_number']),$refrence_year,$sectors[$rows['sector_of_operation']], $dgbgV['panchayat'][$rows['gram_panchayat_code']], $dgbgV['village'][$rows['village_code']], $banckarray[trim($rows['affiliated_union_federation_name'])],$rows['members_of_society'], count($rows['area_of_operation_level']),$buildingTypes[$rows['cooperative_registration_pacs'][0]['building_type']], $astatus[$rows['is_approved']], trim($rows['contact_person']), trim($designations[$rows['designation']]),$rows['mobile']]; 
             $i++;
             }
