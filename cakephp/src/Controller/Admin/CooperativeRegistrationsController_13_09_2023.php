...
class CooperativeRegistrationsController extends AppController
{
    ...
    public function beforeFilter(Event $event)
    {
        ...
        $this->Auth->allow(['getDistricts','getBlocks','getGp','getVillages','getUrbanLocalBodies','getUrbanLocalBody','getLocalityWard','getPrimaryActivity','getdccb','getfederationlevel','approval','bulkapproval','getUrbanRural','generateUniqueNumber','getBank','getRegistrationAuthority','getSocietyData']);
    }
    ...
    public function add()
    {
        ...
        $sub_query = $this->Employees->find('all')
                                    ->select(['department_id','dob'=>'max(dob)'])
                                    ->group(['department_id']);
                                    //->toArray();

        $dob_data = $this->Employees->find('all')
                        ->select(['name','email','department_id','dob'])
                        ->where(['(department_id,dob) IN'=> $sub_query])
                        ->order(['id'])->toArray();
                                    

        echo '<pre>';
        print_r($dob_data);die;*/
        ...
        if($this->request->is('post')){
             ...
             if($data['is_draft'] == 0)
           {
            ...
            if($data['sector_of_operation'] == '11') 
            {
                ...
                $data['cooperative_registrations_sugar']['product_produced'] = implode(',',$data['cooperative_registrations_wocoop']['product_produced']);
            ...
            //save dynamic added questions data

            $questions = $this->ServiceQuestions->find('list',['keyField'=>'id','valueField'=>'question_type'])->where(['status'=>1,'primary_activity'=>$data['sector_of_operation']])->toarray();
            $answers = $this->ServiceAnswers->find('list',['keyField'=>'id','valueField'=>'answer_name'])->where(['status'=>1])->toarray();
            
            $service_data = [];
            if(!empty($data['cooperative_registrations_services']))
            {

                foreach ($data['cooperative_registrations_services'] as $question_id => $answer_id) {
                    $service_data[] = [
                        'service_question_id'           =>  $question_id,
                        'value_id'                      =>  $answer_id, //$answer_id,// answer text
                        'value_text'                    =>  ($questions[$question_id] == 2 ? '' : ( $questions[$question_id] == 3 ? $answer_id : $answers[$answer_id])) 
                    ];
                }
                $data['cooperative_registrations_services'] = [];

                $data['cooperative_registrations_services'] = $service_data;

            }
               
            $CooperativeRegistration = $this->CooperativeRegistrations->patchEntity($CooperativeRegistration, $data,['associated' => ['SocietyImplementingSchemes','CooperativeReceivedBenefits','CooperativeRegistrationsMarketing','CooperativeRegistrationsHousing','CooperativeRegistrationsTransport','CooperativeRegistrationsCreditThrift','CooperativeRegistrationsConsumer','CooperativeRegistrationsLabour','CooperativeRegistrationsLands','CooperativeRegistrationsProcessing','CooperativeRegistrationsHandloom','CooperativeRegistrationsAgriculture','CooperativeRegistrationsTribal','CooperativeRegistrationsTourism','CooperativeRegistrationsHandicraft','CooperativeRegistrationsEducation','CooperativeRegistrationsJute','CooperativeRegistrationsLivestock','CooperativeRegistrationsSericulture','CooperativeRegistrationsSugar','CooperativeRegistrationsServices']]);
       ...
       $this->set('sector_operations',$sector_operations); 
        ...
        $userPrimary =$this->UserPrimary->find('list',['keyField'=>'user_id','valueField'=>'primary_activity_id'])->toArray();

        $is_sugar_deo = '0';

        if(!empty($userPrimary) && in_array('11',$userPrimary))
        {
            $this->loadModel('FederationSugarMills');
            $sugar_mill_name = $this->FederationSugarMills->find('list',['keyField'=>'id','valueField'=>'sugar_mill_name'])->order(['sugar_mill_name'=>'ASC'])->toArray();
            $sugar_mill_name = array_map('trim', $sugar_mill_name);
            $this->set('sugar_mill_name',$sugar_mill_name);
            $is_sugar_deo = '1';
        }


        $this->set(compact('CooperativeRegistration','states','CooperativeSocietyTypes','areaOfOperationsUrban','areaOfOperationsRural','PrimaryActivities','SecondaryActivities','PresentFunctionalStatus','years','state_code','userPrimary','is_sugar_deo'));
    ...
    public function edit($id = null)
    {
        ...
        $CooperativeRegistration = $this->CooperativeRegistrations->get($id, [
            'contain' => ['CRMS','CooperativeRegistrationsContactNumbers','CooperativeRegistrationsEmails','CooperativeRegistrationsLands','CooperativeRegistrationPacs'=>['sort'=>['id'=>'desc']],'CooperativeRegistrationDairy'=>['sort'=>['id'=>'desc']],'CooperativeRegistrationFishery'=>['sort'=>['id'=>'desc']],'AreaOfOperationLevel','CooperativeRegistrationsHousing','CooperativeRegistrationsMarketing','CooperativeRegistrationsCreditThrift','CooperativeRegistrationsConsumer','CooperativeRegistrationsLabour','CooperativeRegistrationsTransport','CooperativeRegistrationsProcessing','CooperativeRegistrationsHandloom','CooperativeRegistrationsAgriculture','CooperativeRegistrationsTribal','CooperativeRegistrationsTourism','CooperativeRegistrationsHandicraft','CooperativeRegistrationsEducation','CooperativeRegistrationsJute','CooperativeRegistrationsSugar','CooperativeRegistrationsServices']
        ]);
        
        $service_answers = $this->CooperativeRegistrationsServices->find('list',['keyField'=>'service_question_id','valueField'=>'value_id'])->where(['cooperative_registration_id'=>$id])->toArray();
        $this->set('service_answers',$service_answers);

        $dep_questions = $this->ServiceAnswers->find('list',['keyField'=>'id','valueField'=>'child_question_id'])->where(['child_question_id !='=>''])->toArray();

        $display_dep_questions = []; 
        if(!empty($dep_questions))
        {
            foreach ($dep_questions as $ans_id => $q_id) {
            
                if(in_array($ans_id,$service_answers))
                {
                    $display_dep_questions[] =  $q_id;
                }
            }
        }
        
        // echo '<pre>';
        // print_r($display_dep_questions);die;
        $this->set('display_dep_questions',$display_dep_questions);
        
        // echo '<pre>';
        // print_r($service_answers);die;

        $services = $this->services($CooperativeRegistration->sector_of_operation);
        ...
        if ($this->request->is(['patch', 'post', 'put'])){
            ...
            // echo '<pre>';
            // print_r($data['cooperative_registrations_sugar']);die;
  ...
  $this->viewBuilder()->setLayout('ajax');
   ...
   $this->set('hr2',$hr2);
    ...
    public function draft()
    {
        ...
        $registrationQuery = $this->CooperativeRegistrations->find('all', [
            'order' => ['CooperativeRegistrations.id' => 'DESC'],
            'conditions' => [$searchString,'CooperativeRegistrations.created_by'=>$this->request->session()->read('Auth.User.id')],
            'contain' => ['PrimaryActivities']
        ])->where(['CooperativeRegistrations.is_draft'=>1,'CooperativeRegistrations.status'=>1]);
    ...
    public function getPrimaryActivity(){
        $this->viewBuilder()->setLayout('ajax');
            if($this->request->is('ajax')){
                ...
                $pactivities = $this->PrimaryActivities->find('all')->where(['sector_of_operation'=>$sector_operation,'status'=>1])->order(['orderseq'=>'ASC']);
   ...
   public function getdccb()
    ...
    public function services($primary_activity)
    {
        $this->loadModel('ServiceQuestions');
        $this->loadModel('ServiceAnswers');
        //=====================================
        $searchString = '';
        $questions = $this->ServiceQuestions->find('all', [
            'order' => ['ServiceQuestions.orderseq' => 'ASC'],
            //'conditions' => [$searchString],
            'contain' => ['ServiceAnswers']
        ])->where(['ServiceQuestions.status'=>1,'ServiceQuestions.primary_activity'=>$primary_activity])->toArray();

        $question_dropdown = [];
        foreach($questions as $question)
        {
            if(!empty($question['master_table']))
            {
                $master = $question['master_table'];
                $this->loadModel($master);
                $question_dropdown[$question['id']] =$this->$master->find('list',['keyField'=>'id','valueField'=>$question['master_column_name']])->where(['status'=>1])->toArray();
            }
        }
        
        $data['question'] = $questions;
        $data['dropdown'] = $question_dropdown;
        return $data;
        //=====================================

        // $questions = $this->ServiceAnswers->find('all')->where(['primary_activity'=>$primary_activity,'status'=>'1'])->order(['orderseq'=>'ASC'])->toArray();
        // $answers = $this->ServiceQuestions->find('all')->where(['primary_activity'=>$primary_activity,'status'=>'1'])->order(['orderseq'=>'ASC'])->toArray();
        // echo '<pre>';
    ...
    {
        $this->viewBuilder()->setLayout('ajax');
        $sector = $this->request->data['society_type'];
        $this->set('sector',$sector);

        $services = $this->services($this->request->data['society_type']);
        $this->set('services',$services);
    ...
    public function logDetail($id)
    {
        ...
        $registrationQuery = $this->CooperativeRegistrationsLogs->find('all', [
            ...
            'conditions' => [$searchString,$search_condition2,$search_condition3],
            'contain' => ['PrimaryActivities']
        ])->where(['CooperativeRegistrationsLogs.is_draft'=>0,'CooperativeRegistrationsLogs.is_approved'=>0,'CooperativeRegistrationsLogs.status'=>1,'CooperativeRegistrationsLogs.cooperative_registration_id'=>$id]);
...
// mrig end
    ...
    public function getSocietyData()
    {
        $this->viewBuilder()->setLayout('ajax');
        if($this->request->is('ajax')){
            $id=$this->request->data('sugar_id');
            $this->loadModel('FederationSugarMills');

           $data = $this->FederationSugarMills->get($id);

           if($data->district_code != 0)
           {
            
           }
           echo $data->district_code;
