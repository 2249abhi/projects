...
class SugarMillsFederationController extends AppController
{
    ...
    //die;
            ...
            $this->set('Districts',$Districts);
                ...
                $asugarmeals_reored = $this->FederationSugarMills->find('all')->where([$condtion_district,$search_condition])->contain(['States'])->order(['States.name'=>'ASC']);    
           

                $this->paginate = ['limit' => 20];
                $district_codes = $this->paginate($asugarmeals_reored);
                ...
                { 
                    ...
                    $ExportResultData= $this->FederationSugarMills->find('all')->where([$condtion_district,$search_condition])->contain(['States'])->order(['States.name'=>'ASC'])->toarray();  
                    ...
                    //   $pacs=
                   $sugar_mill_name= str_replace('','',$this->strClean(trim($rows['sugar_mill_name'])));
                    $data[] = [$i,  $sugar_mill_name, $stateOption[$rows['state_code']]?? 0, $Districts[$rows['district_code']],$rows['phone_number_of_contact_person'],$rows['email'],$rows['name_of_contact_person'] ,$rows['short_name'] ?? 0 , $rows['operational_status'], $rows['ownership_status'], $rows['actual_capacity'],$rows['ethanol_capacity'],$rows['cogen_capacity'],$rows['profit_loss']]; 
                    $i++;
                    }
