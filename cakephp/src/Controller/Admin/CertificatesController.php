...
class CertificatesController extends AppController
{
    ...
    public function index()
    {
        ...
        $certificates = $this->Certificates->find('all', [
            ...
            //'contain' => ['Users'],
            'conditions' => [$searchString,$search_condition2,$search_condition3,'Certificates.status'=>1,'Certificates.type IN'=>[1,3,4],'Certificates.title !='=>'']
        ]);//->toArray()
    ...
    public function certificate()
    {
       ...
       $this->loadModel('States');
        ...
        if(!empty($this->request->data('certificate_district')))
        {//filter with interns district array
            $district_code = $this->request->data('certificate_district');
        }
        ...
        $pacs_data['total_data_entered'] = 0;
        // if(!empty($nodal_data_entry_ids))
        // {
            $pacs_data['total_data_entered'] = $this->CooperativeRegistrations->find('all', [
                'order' => ['created' => 'DESC'],
                'conditions' => ['is_draft'=>0,'is_approved'=>1,'status'=>1,'sector_of_operation IN' => [1,59,20,22],'state_code'=>$this->request->session()->read('Auth.User.state_code'),'district_code'=>$district_code]
            ])->count();//,'created_by IN'=>$nodal_data_entry_ids    
        //}
        ...
        $dairy_data['total_data_entered'] = 0;

        // if(!empty($nodal_data_entry_ids))
        // {
            $dairy_data['total_data_entered'] = $this->CooperativeRegistrations->find('all', [
                'order' => ['created' => 'DESC'],
                'conditions' => ['is_draft'=>0,'is_approved'=>1,'status'=>1,'sector_of_operation IN' => [9,37],'state_code'=>$this->request->session()->read('Auth.User.state_code'),'district_code'=>$district_code]
            ])->count();
        //}

        $fishery_data['total_record'] = array_sum($district_nodal_total_without_state['fishery']) ?? 0;
        
        $fishery_data['total_data_entered'] = 0;

        // if(!empty($nodal_data_entry_ids))
        // {
            $fishery_data['total_data_entered'] = $this->CooperativeRegistrations->find('all', [
                'order' => ['created' => 'DESC'],
                'conditions' => ['is_draft'=>0,'is_approved'=>1,'status'=>1,'sector_of_operation IN' => [10,43],'state_code'=>$this->request->session()->read('Auth.User.state_code'),'district_code'=>$district_code]
            ])->count();
        //}
        $query =$this->States->find('list',['keyField'=>'state_code','valueField'=>'name','conditions'=>['flag'=>1],'order' => ['name' => 'ASC']]);
        $query->hydrate(false);
        ...
        //$sign_district_names = implode(', ',array_values(array_intersect_key($districts, array_flip(array_keys($arr_sign_districts)))));

        //$arr_sign_districts = $this->CooperativeRegistrations->find('all',['conditions'=>['operational_district_code'=>$this->request->session()->read('Auth.User.district_code')]])->select(['district_code'])->toArray();
        ...
        $certificate_districts = array_intersect_key($districts, array_flip(array_keys($arr_sign_districts)));
        
        $this->set(compact('pacs_data','dairy_data','fishery_data','districts','nodal_entry_id','sign_district_names','certificate_districts','district_code'));
    ...
    public function pacsCertificate()
    {
        ...
        $pacs_data['total_data_entered'] = $this->CooperativeRegistrations->find('all', [
            'order' => ['created' => 'DESC'],
            'conditions' => ['is_draft'=>0,'is_approved'=>1,'status'=>1,'sector_of_operation IN' => [1,59,20,22],'state_code'=>$this->request->session()->read('Auth.User.state_code'),'operational_district_code'=>$this->request->session()->read('Auth.User.district_code')]
        ])->count();//,'created_by IN'=>$nodal_data_entry_ids
        ...
        $this->set(compact('pacs_data','districts','nodal_entry_id'));

    }


    //generate district base custom pacs/dairy/fishery certificate 
       ...
       $this->loadModel('States');
        ...
        if ($this->request->is('post')) {
            ...
            if(!empty($data['district_code']))
            {
                ...
                // $this->Flash->error(__('Sorry unable to generate'));
                // $this->redirect($this->referer());
            }
        }
        ...
        if($role_id == 8)
        {
            if($this->request->session()->read('Auth.User.primary_act_code') == 1)
            {
                $nodal_data_entry_ids = $this->Users->find('list',['keyField'=>'id','valueField'=>'name'])->where(['status'=>1,'state_code'=>$this->request->session()->read('Auth.User.state_code'),'district_code'=>$district_code])->toArray();//,'created_by'=>$this->request->session()->read('Auth.User.id')
            } else {
                $nodal_data_entry_ids = $this->Users->find('list',['keyField'=>'id','valueField'=>'name'])->where(['status'=>1,'created_by'=>$this->request->session()->read('Auth.User.id'),'state_code'=>$this->request->session()->read('Auth.User.state_code'),'district_code'=>$district_code])->toArray();//
            }
        ...
        $pacs_data['total_data_entered'] = $this->CooperativeRegistrations->find('all', [
            'order' => ['created' => 'DESC'],
            'conditions' => ['is_draft'=>0,'is_approved'=>1,'status'=>1,'sector_of_operation IN' => [1,59,20,22],'state_code'=>$this->request->session()->read('Auth.User.state_code'),'operational_district_code'=>$district_code]
        ])->count();//,'created_by IN'=>$nodal_data_entry_ids
        ...
        $dairy_data['total_data_entered'] = $this->CooperativeRegistrations->find('all', [
            'order' => ['created' => 'DESC'],
            'conditions' => ['is_draft'=>0,'is_approved'=>1,'status'=>1,'sector_of_operation IN' => [9,37],'state_code'=>$this->request->session()->read('Auth.User.state_code'),'operational_district_code'=>$district_code]
        ])->count();
        ...
        $fishery_data['total_data_entered'] = $this->CooperativeRegistrations->find('all', [
            'order' => ['created' => 'DESC'],
            'conditions' => ['is_draft'=>0,'is_approved'=>1,'status'=>1,'sector_of_operation IN' => [10,43],'state_code'=>$this->request->session()->read('Auth.User.state_code'),'operational_district_code'=>$district_code]
        ])->count();
        ...
        $this->set(compact('pacs_data','dairy_data','fishery_data','districts','nodal_entry_id','district_code'));
