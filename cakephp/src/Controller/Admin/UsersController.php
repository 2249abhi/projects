...
class UsersController extends AppController
{
    ...
    public function index()
    {
        ...
        {
			$created_by= $this->request->session()->read('Auth.User.created_by');
            $search_condition3  = "id = '" . $created_by . "'";
        }
        ...
        else if($loginUser['role_id']==56){
            // $users = $this->paginate($this->Users->find('all')->where(['role_id IN'=>['7'], 'created_by'=>$loginUser['id']])); 
            $users = $this->paginate($this->Users->find('all')->where([ $searchString,'state_code'=>$loginUser['state_code']])->order(['Users.id'=>'DESC']));  
		...
		else if($loginUser['role_id'] ==15){ 
            $users = $this->paginate($this->Users->find('all')->where(['parent_id'=>$this->request->session()->read('Auth.User.id'),'role_id IN'=>['13'], $searchString])->order(['Users.id'=>'DESC']));  
        }
		else if($loginUser['role_id'] ==21 || $loginUser['role_id'] ==24){
			$created_by= $this->request->session()->read('Auth.User.id');

			$users = [];
			if(!empty($created_by)){ 
				$users = $this->paginate($this->Users->find('all')->where(['created_by'=>$created_by, $searchString])->order(['Users.id'=>'DESC'])); 
			}
        ...
        if($loginUser['role_id']==2){          
            $roles=$this->Roles->find('list',['keyField'=>'id','valueField'=>'name'])->order(['name'=>'ASC'])->toArray(); 
        }
		else if($loginUser['role_id']==21){          
            $roles=$this->Roles->find('list',['keyField'=>'id','valueField'=>'name'])->order(['name'=>'ASC'])->where(['id IN'=>[15]])->toArray(); 
        }
		else{
			 $roles=$this->Roles->find('list',['keyField'=>'id','valueField'=>'name'])->order(['name'=>'ASC'])->where(['id IN'=>[7,8,11,13,15,24,25]])->toArray();
		}
        ...
        // $query = $this->UserPrimary->find('all')->select(['user_id','activity' => 'group_concat(primary_activity_id)'])
        //          ->group('user_id')->hydrate(false)->toArray();

        $arr_u_primary = $this->UserPrimary->find('all')->select(['user_id','activity' => 'group_concat(primary_activity_id)'])->group('user_id')->hydrate(false)->toArray();
        $u_primary = [];
        foreach($arr_u_primary as $p_key => $pvalue)
        {
            $u_primary[$pvalue['user_id']] = $pvalue['activity'];
        }

        $this->set(compact('users','districts','states','all_user','roles','pact','u_primary'));
    }
    ...
    public function view($id = null)
    {
        $user = $this->Users->get($id, [
          ...
          $states=$this->States->find('list',['keyField'=>'state_code','valueField'=>'name'])->order(['name'=>'ASC'])->where(['flag'=>1])->toArray();
            ...
            $all_user= $this->Users->find('list',['keyField'=>'id','valueField'=>'name'])->toarray();
            $primary_activity_array= $this->UserPrimary->find()->where(['user_id'=>$id])->extract('primary_activity_id')->toarray();         
            $this->set(compact('districts','states','pact','all_user','primary_activity_array'));
            $this->set('user', $user);
    }
    ...
    public function add()
    {
        ...
        if ($this->request->is('post')) {
            ...
            if ($this->Users->save($user)) {
                    $primary_activity_arrr=$data['primary_act_code_id'];
                   // print_r($primary_activity_arrr);
                    foreach($primary_activity_arrr as $key=>$value)
                        {   
                            if($value !='')
                            {
                             $user_pdata = $this->UserPrimary->newEntity();
                            $userdata['user_id']=$user['id'];
                            $userdata['primary_activity_id']=$value;
                            $user_pdata = $this->UserPrimary->patchEntity($user_pdata, $userdata);
                            $this->UserPrimary->save($user_pdata);
                            }
        ...
        if($loginUser['role_id']==2){
            //$roles = $this->Users->Roles->find('list', ['limit' => 200])->where(['Roles.id IN '=>['8','11','13','15']]);
            $roles = $this->Users->Roles->find('list', ['limit' => 200])->order(['sequence'=>'ASC']);
        }
        else if($loginUser['role_id']==8){
            $roles = $this->Users->Roles->find('list', ['limit' => 200])->where(['Roles.id IN '=>['7']])->order(['sequence'=>'ASC']);;
        }
		else if($loginUser['role_id']==11){
            $roles = $this->Users->Roles->find('list', ['limit' => 200])->where(['Roles.id IN '=>['8','7']])->order(['sequence'=>'ASC']);;
        }
		else if($loginUser['role_id']==15){
            $roles = $this->Users->Roles->find('list', ['limit' => 200])->where(['Roles.id IN '=>['13']])->order(['sequence'=>'ASC']);;
        }else if($loginUser['role_id']==24){
            $roles = $this->Users->Roles->find('list', ['limit' => 200])->where(['Roles.id IN '=>['25']]);
        }
        else{
            $roles = $this->Users->Roles->find('list', ['limit' => 200]);    
        }

        if($loginUser['role_id']==24)
        {
            $states=$this->States->find('list',['keyField'=>'state_code','valueField'=>'name'])->order(['name'=>'ASC'])->where(['state_code'=>$loginUser['state_code'],'flag'=>1]);
        } else {
		...
		if($loginUser['role_id']==1){
			$nationalList = $this->Users->find('list', ['limit' => 200])->where(['role_id'=>15]);
		}
		else{
			$nationalList = $this->Users->find('list', ['limit' => 200])->where(['role_id'=>13]);
		}
        
        
        
        $this->set(compact('user', 'roles','states','districts','loginUser','pactivities','nationalList'));
    }
    ...
    public function edit($id = null)
    {
        ...
        if ($this->request->is(['patch', 'post', 'put'])) {
            ...
            if ($this->Users->save($user)) {
                ...
                $userp_array=$this->UserPrimary->find('list', ['limit' => 200])->where(['UserPrimary.user_id'=>$user['id']])->toarray();
                if(count($userp_array) >0)
                {
                     $this->UserPrimary->deleteall(['UserPrimary.user_id'=>$user['id']]);
                }
                // print_r($primary_activity_arrr);
                 foreach($primary_activity_arrr as $key=>$value)
                     {   
        ...
        if($loginUser['role_id'] ==2){
            ...
            $roles = $this->Users->Roles->find('list', ['limit' => 200])->where(['Roles.id IN '=>['13']]);
        }
        ...
        $pactivities=$this->PrimaryActivities->find('list',['keyField'=>'id','valueField'=>'name'])->order(['name'=>'ASC'])->where(['status'=>1]);
        $pactivitiesarray=$this->UserPrimary->find()->order(['primary_activity_id'=>'ASC'])->where(['user_id'=>$id])->extract('primary_activity_id')->toarray();
        $this->set(compact('user', 'roles', 'states','districts', 'loginUser', 'pactivities','nationalList','role_id','pactivitiesarray'));
