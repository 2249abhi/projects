 ...
 */
class GlobalLeadersController extends AppController
{

    public function initialize()
    {
        parent::initialize();
        $this->loadModel('GlobalLeaders'); 
    ...
    public function index()
    {
        ...
        if($this->request->is('get')){
            if(!empty($this->request->query['export_excel'])) {
                $queryExport = $this->GlobalLeaders->find('all',[
                    'conditions'=>[$searchString],
                    'limit' => $page_length,
                    'page'=> $page,
                    'order' => ['GlobalLeaders.id' => 'asc'],
                ]);
        ...
        $rolesQuery = $this->GlobalLeaders->find('all', [
            'contain' => [],
            'order' => ['GlobalLeaders.id' => 'asc'],
            'conditions' => [$searchString]
        ]);
    ...
    {
        $role = $this->GlobalLeaders->get($id, [
            'contain' => []
        ]);
    ...
    public function add()
    {
        $role = $this->GlobalLeaders->newEntity();
        if ($this->request->is('post')) {
            $role = $this->GlobalLeaders->patchEntity($role, $this->request->getData());

            if(isset($this->request->data['image']['error']) && $this->request->data['image']['error']==0){
                $photo = $this->uploadFiles('global_leader', $this->request->data['image']); 
            ...
            if ($this->GlobalLeaders->save($role)) {
                $this->Flash->success(__('The global leader has been saved.'));

                return $this->redirect(['action' => 'index']);
            }
            $this->Flash->error(__('The global leader could not be saved. Please, try again.'));
        }
    ...
    public function edit($id = null)
    {
        ...
        if ($this->request->is(['patch', 'post', 'put'])) {
            $role = $this->GlobalLeaders->patchEntity($role, $this->request->getData());
             ...
             if(isset($this->request->data['image']['error']) && $this->request->data['image']['error']==0){
                $photo = $this->uploadFiles('global_leader', $this->request->data['image']); 
                //echo "<pre>"; print_r($this->request->data['image']);exit;
                $role->image = $photo['filename'];
            }else{
                $role->image = @$this->request->data['image_old']; 
            }
    ...
    public function delete($id = null)
    {
        ...
        $role = $this->GlobalLeaders->get($id);
        if ($this->GlobalLeaders->delete($role)) {
            $this->Flash->success(__('The global leader has been deleted.'));
        } else {
            $this->Flash->error(__('The global leader could not be deleted. Please, try again.'));
        }
