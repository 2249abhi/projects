 ...
 */
$this->assign('title',__('Video ( ' . $doc_type->title . ' ) '));
$this->Breadcrumbs->add(__(Videos),['action'=>'index']);
$this->Breadcrumbs->add(__('Video ( ' . $doc_type->title . ' ) '));
?>
        ...
        <div class="col-md-12">
           <div class="box box-solid">
              <div class="box-header with-border">
                 ...
                 <h3 class="box-title"><?= __('Video ( ' . $doc_type->title . ' ) ') ?></h3>
              ...
              <div class="box-body">
                <table class="table table-striped table-bordered">
                    <tr>
                      ...
                      <td><?= html_entity_decode($doc_type->title) ?></td>
					  <th scope="row"><?= __('Vido Link') ?></th>
                      <td><?= html_entity_decode($doc_type->video_link) ?></td>
                    </tr>
                    <tr>
						<th><?= __('Video Date') ?></th>
						<td><?= date('Y-m-d H:i A',strtotime($doc_type->video_date)) ?></td>
						<th scope="row"><?= __('Video Order') ?></th>
						<td><?= h($doc_type->video_order) ?></td>
