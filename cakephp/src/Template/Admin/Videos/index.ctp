 ...
 */
$this->assign('title',__('Video'));
$this->Breadcrumbs->add(__('Video'));
      ...
      <div class="col-md-12">
        ...
        <div class="box box-primary">
          <div class="box-header">
              <h3 class="box-title"><?= __('Search - Video'); ?></h3>
          </div> 
          <?= $this->Form->create('searchBanner',['id' =>'searchBanner','type'=>'get']); ?>
            <div class="box-body table-responsive">
              ...
              <div class="row2">    
                ...
                <div class="col-md-1"><label for="page_name">Video Title</label></div>
                <div class="col-md-3">
                ...
                <div class="col-md-1">
                  ...
                  <?php } else { ?>
                    <?= $this->Html->link(__('Reset'),['controller'=>'Videos','action'=>'index'],['class'=>'btn btn-danger']); ?>
                  <?php }?>
                </div>
          ...
          <?= $this->Form->create('page_number',['id'=>'role_list','type'=>'get','class'=>'form-inline']); ?>
            <div class="box-header">
              ...
              <div class="box-tools">
                ...
                <?=$this->Html->link(
                    __('<i class="glyphicon glyphicon-plus"></i> New Video'),
                    ['action' => 'add'],
                    ['class' => 'btn btn-success', 'escape' => false, 'title' => __('Add New Video')]
                );?>                
              </div>              
            </div>
            <div class="clearfix"></div>
            ...
            <table class="table table-hover table-bordered">
              <thead>
				...
				<?php $video_cat = [1=>'Speeches',2=>'Press Conference'];?>
                    ...
                    <th scope="col"><?= $this->Paginator->sort('video_cat',"Video Category") ?></th>
                    <th scope="col"><?= $this->Paginator->sort('video_link') ?></th>
                    <th scope="col"><?= $this->Paginator->sort('video_date',"Video Date") ?></th>
                    <th scope="col"><?= $this->Paginator->sort('video_order',"Video Order") ?></th>
              ...
              <tbody>
				...
				foreach ($document_types as $role): ?>
                  <tr>
                    ...
                    <td><?= html_entity_decode($role->title); ?></td>
                    <td><?= $role->video_cat; //($video_cat[$role['video_cat']]); ?></td>
                    <td><?= html_entity_decode($role->video_link); ?></td>
                    <td><?= date('Y-m-d',strtotime($role->video_date)) ?></td>
                    <td><?= h($role->video_order) ?></td> 
                    ...
                    <td class="actions">
                        ...
                        <?= $this->Form->postLink('<i class="fa fa-trash-o"></i>', ['action' => 'delete', $role->id], ['confirm' => __('Are you sure you want to delete this Record?', $role->id),'class' => 'btn btn-danger btn-xs', 'title' => __('Delete'), 'escape' => false]) ?>
                      </td>
                    </tr>
                    <?php $i++; endforeach; ?>
