 ...
 */
$this->assign('title',__('Multimedia Sub Categories'));
$this->Breadcrumbs->add(__('Multimedia Sub Categories'));
?>
  <!-- Main content -->
      ...
      <?= $this->Flash->render() ; ?>
        <div class="box box-primary">
          <div class="box-header">
            <h3 class="box-title"><?= __('List of') ?> <?= __('Multimedia Sub Categories') ?></h3>
            <div class="box-tools">
              <?=$this->Html->link(
                __('<i class="glyphicon glyphicon-plus"></i> Add New Multimedia Sub Category'),
                ['action' => 'add'],
                ['class' => 'btn btn-success', 'escape' => false, 'title' => __('Add New Multimedia Sub Category')]
              );?>
            </div>
          </div>
          <!-- /.box-header -->
...
<!--                    <td><?= $galleryCategory->has('article') ? $this->Html->link($galleryCategory->article->title, ['controller' => 'Articles', 'action' => 'view', $galleryCategory->article->id]) : '' ?></td>-->
                    ...
                    <td><?= h(date('Y-m-d',strtotime($galleryCategory->created))); ?></td>
                    <td class="actions">
