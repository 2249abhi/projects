...
.society_member_chart { background:#fff;}
...
Highcharts.chart('societymemberchart', {
    ...
    xAxis: [{
        categories: ['Agriculture and Allied', 'Agro Processing / Industrial', 'Bee Farming', 'Consumer', 'Credit & Thrift', 'Dairy', 'Educational & Training',
            'Fishery', 'Handicraft and Weaver', 'Handloom Textile & Weavers'],
        crosshair: true
    }],
    yAxis: [{ // Primary yAxis
        title: {
    ...
    series: [{
        ...
        type: 'column',
        yAxis: 1,
        data: [<?= ($society_data['agriculture_and_allied_cooperative'] ?? 0) .', '.($society_data['agro_Processing_industrial_cooperative'] ?? 0).', '.($society_data['bee_farming_cooperative'] ?? 0).', '.($society_data['consumer_cooperative'] ?? 0).', '.($society_data['credit_thrift_society'] ?? 0).', '.($society_data['dairy_cooperative'] ?? 0).', '.($society_data['educational_and_training_cooperatives'] ?? 0).', '.($society_data['fishery_cooperative'] ?? 0).', '.($society_data['handicraft_cooperative'] ?? 0).', '.($society_data['handloom_textile_weavers_cooperative'] ?? 0) ?>],
        tooltip: {
            valueSuffix: ''
        }
        ...
        type: 'spline',
        yAxis: 2,
        data: [<?= ($member_data[30] ?? 0) .', '.($member_data[31] ?? 0).', '.($member_data[79] ?? 0).', '.($member_data[80] ?? 0).', '.($member_data[18] ?? 0).', '.($member_data[9] ?? 0).', '.($member_data[84] ?? 0).', '.($member_data[10] ?? 0).', '.($member_data[14] ?? 0).', '.($member_data[13] ?? 0) ?>],
        marker: {
            enabled: false
        },
...
Highcharts.chart('societymemberchart2', {
    ...
    xAxis: [{
        categories: ['Jute and Coir','Labour','Livestock & Poultry','Miscellaneous','Miscellaneous Credit','Multipurpose','Primary Agricultural Credit (PACS)','Primary Housing','Primary Marketing','Sericulture'],
        crosshair: true
    }],
    yAxis: [{ // Primary yAxis
    ...
    series: [{
        ...
        data: [<?= ($society_data['jute_and_coir_cooperative'] ?? 0) .', '.($society_data['labour_cooperative'] ?? 0).', '.($society_data['livestock_poultry_cooperative'] ?? 0).', '.($society_data['miscellaneous'] ?? 0).', '.($society_data['miscellaneous_credit_cooperative_society'] ?? 0).', '.($society_data['multipurpose_cooperative'] ?? 0).', '.($society_data['primary_agricultural_credit_society_pacs'] ?? 0).', '.($society_data['primary_housing_cooperative_society'] ?? 0).', '.($society_data['primary_marketing_cooperative_society'] ?? 0).', '.($society_data['sericulture_cooperative'] ?? 0) ?>],
        tooltip: {
            valueSuffix: ''
        }
    }, {
        ...
        type: 'spline',
        yAxis: 2,
        data: [<?= ($member_data[90] ?? 0) .', '.($member_data[51] ?? 0).', '.($member_data[54] ?? 0).', '.($member_data[29] ?? 0).', '.($member_data[35] ?? 0).', '.($member_data[16] ?? 0).', '.($member_data[1] ?? 0).', '.($member_data[47] ?? 0).', '.($member_data[82] ?? 0).', '.($member_data[96] ?? 0) ?>],
        marker: {
            enabled: false
        },
...
Highcharts.chart('societymemberchart3', {
    ...
    xAxis: [{
        categories: ['Social Welfare and Cultural','Sugar Mills','Tourism','Transport','Tribal-SC/ST','Urban Cooperative Bank (UCB)','Women Welfare'],
        crosshair: true
    }],
    yAxis: [{ // Primary yAxis
    ...
    series: [{
        ...
        data: [<?= ($society_data['social_welfare_and_cultural_cooperative'] ?? 0) .', '.($society_data['sugar_mills_cooperative'] ?? 0).', '.($society_data['tourism_cooperative'] ?? 0).', '.($society_data['transport_cooperative'] ?? 0).', '.($society_data['tribal_SC_ST_cooperative'] ?? 0).', '.($society_data['urban_cooperative_bank_ucb'] ?? 0).', '.($society_data['women_welfare_cooperative_society'] ?? 0) ?>],
        tooltip: {
            valueSuffix: ''
        }
        ...
        type: 'spline',
        yAxis: 2,
        data: [<?= ($member_data[98] ?? 0) .', '.($member_data[11] ?? 0).', '.($member_data[99] ?? 0).', '.($member_data[68] ?? 0).', '.($member_data[102] ?? 0).', '.($member_data[7] ?? 0).', '.($member_data[15] ?? 0) ?>],
        marker: {
            enabled: false
        },
