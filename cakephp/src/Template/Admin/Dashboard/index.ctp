...
<section class="content Cooperative Registration index box-container-block" style="position:relative;">
    ...
    <div class="row">
        <div>
            <h4 class="mn_dash" style="margin-top:0 !important;">Dashboard</h4>
        </div>
    </div>

    <div class="row society_member_chart">
    <figure class="highcharts-figure col-md-4">
        <div id="societymemberchart"></div>
        </figure>
        <figure class="highcharts-figure col-md-4">
    <div id="societymemberchart2"></div>
</figure>
<figure class="highcharts-figure col-md-4">
    <div id="societymemberchart3"></div>
</figure>
    </div>
    ...
    <div class="row box pt_x">
        ...
        <div
            style="display:<?= in_array($this->request->session()->read('Auth.User.role_id'),[7,8]) ? 'none' : 'block' ?>;">
            <div class="col-sm-4">
                <div id="gp_pie_graph" class="w100">
                </div>
                <p class="highcharts-description"
                    style="text-align:center;font-weight: bold;background-color: #ccc;padding:10px;">
   ...
   // } ?>
    ...
    <div class="row col-sm-12 box"
        style="text-align:center;margin-left:0px;display:<?= in_array($this->request->session()->read('Auth.User.role_id'),[7,8]) ? 'none' : 'block' ?>;">
        <?php //$this->Html->link(__('All India Report'),['controller'=>'all-india-reports','action'=>'index']); ?>
    ...
    <div class="modal-dialog">
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <p>1).<img style="height:20px;" src="<?php echo $this->Url->build('/img/new.png');?>" /> This is to
                    inform all the Nodal Officers,Data Entry Operators and Other concerned stake hoder that NDC Portal
                    services for Updation and Upgradation shall not be avaiable between 1:30 PM to 2:30 PM, Today due to
                    new Form Upload.
                    <br>
                ...
                <p>2).<img style="height:20px;" src="<?php echo $this->Url->build('/img/new.png');?>" /> Nodal Entry
                    Form has been enabled to enter the target count of verious cooperative societies. All Nodal Officers
                    are requested to fillup before start the data entry of the cooperative Societies.
                    <br>
                    <?php echo $this->Html->link('Click here to view Nodal Data Entry Sample Form', '/files/download/nodal_entry_form.jpg',['download'=>'nodal_entry_form.jpg']); ?>

                </p>
                <p>3).<img style="height:20px;" src="<?php echo $this->Url->build('/img/new.png');?>" /> In order to
                    facilitate further on data verification certificate generation, we have added option for generating
                    and submitting sector wise district wise separate certificates for PACS and Dairy, by respective
                    nodal officer.</p>
                <p>4). <img style="height:20px;" src="<?php echo $this->Url->build('/img/new.png');?>" /> <strong>Steps
                        : How to correct appearing duplicate Cooperative Society Name/Registration No.
                    </strong><?php echo $this->Html->link('Click here to download steps', '/files/download/How to correct duplicate records.pdf',['download'=>'How to correct duplicate records.pdf']); ?>
                </p>
                <p> 5). Important measures have been announced for cooperatives in Union Budget 2023-24. The related
                    contents of Budget Speech are placed herewith for every one's attention.<br />
                ...
                <p> 7). State RCS and District Nodal officers, may kindly see the count of records under the urban to
                    reconcile figures of data, entered by District Nodal Offices. District Nodal Officers are also
                    requested to delete duplicate entries.
                </p>
                <p>8). The Primary Cooperative Society Data is open for final correction, may see the attached documents
    ...
    */
    ...
    Highcharts.chart('pacs_bar', {
        ...
        series: [{
            ...
            data: [
                ['Functional',
                    <?= ($total_functional[1]+$total_functional[20]+$total_functional[22]) ?? 0 ?>
                ],
                ['Non-Functional / Dormant',
                    <?= $non_functional[1]+$non_functional[20]+$non_functional[22] ?? 0 ?>
                ],
                ['Under </br>Liquidation',
                    <?= ($total_under_liquition[1]+$total_under_liquition[20]+$total_under_liquition[22]) ?? 0 ?>
                ]
            ],
            dataLabels: {
                enabled: true,
    ...
    Highcharts.chart('member_chart', {
        ...
        xAxis: {
            categories: ['1900-1923', '1924-1947', '1948-1950', '1951-1970', '1971-1990', '1991-2010',
                '2011-2023'
            ]
        },
        ...
        series: [{
                name: 'PACS',
                data: [<?= $member['1']['1900'] ?? 0 ?>, <?= $member['1']['1924'] ?? 0 ?>,
                    <?= $member['1']['1948'] ?? 0 ?>, <?= $member['1']['1951'] ?? 0 ?>,
                    <?= $member['1']['1971'] ?? 0 ?>, <?= $member['1']['1991'] ?? 0 ?>,
                    <?= $member['1']['2011'] ?? 0 ?>
                ]
            },
            {
                name: 'Dairy',
                data: [<?= $member['9']['1900'] ?? 0 ?>, <?= $member['9']['1924'] ?? 0 ?>,
                    <?= $member['9']['1948'] ?? 0 ?>, <?= $member['9']['1951'] ?? 0 ?>,
                    <?= $member['9']['1971'] ?? 0 ?>, <?= $member['9']['1991'] ?? 0 ?>,
                    <?= $member['9']['2011'] ?? 0 ?>
                ]
            },
            {
                name: 'Fishery',
                data: [<?= $member['10']['1900'] ?? 0 ?>, <?= $member['10']['1924'] ?? 0 ?>,
                    <?= $member['10']['1948'] ?? 0 ?>, <?= $member['10']['1951'] ?? 0 ?>,
                    <?= $member['10']['1971'] ?? 0 ?>, <?= $member['10']['1991'] ?? 0 ?>,
                    <?= $member['10']['2011'] ?? 0 ?>
                ]
            }
        ],
    ...
    Highcharts.chart('so_af_pacs', {
        ...
        series: [{
            name: 'count',
            data: [
                ['Affiliated <br><?php echo $affliation_response["affilated"]; ?><br>(<?php echo $afp; ?> %)',
                    <?php echo $affliation_response["affilated"]; ?>
                ],
                ['Not<br>Affiliated<br><?php echo $affliation_response["not_affilated"]; ?><br>(<?php echo $nafp; ?> %)',
                    <?php echo $affliation_response["not_affilated"]; ?>
                ],
...
//Society Wise TOP 10 Chart

Highcharts.chart('societymemberchart', {
    chart: {
        zoomType: 'xy'
    },
    title: {
        text: 'Sector-wise Society and Member',
        align: 'center'
    },
    xAxis: [{
        categories: [<?php
            echo "'".implode("', '",$sectors[0])."'";
            ?>],
        crosshair: true
    }],
    yAxis: [{ // Primary yAxis
...
Highcharts.chart('societymemberchart2', {
    chart: {
        zoomType: 'xy'
    },
    title: {
    ...
    series: [{
        ...
        type: 'column',
        yAxis: 1,
        //'Jute and Coir','Labour','Livestock & Poultry','Miscellaneous','Miscellaneous Credit','Multipurpose','Primary Agricultural Credit (PACS)','Primary Housing','Primary Marketing','Sericulture'
        data: [<?php
            echo implode(', ',$societies[1]);
            ?>],
        tooltip: {
            valueSuffix: ''
        }
...
Highcharts.chart('societymemberchart3', {
    chart: {
        zoomType: 'xy'
    },
    title: {
    ...
    series: [{
        ...
        type: 'column',
        yAxis: 1,
        //'Social Welfare and Cultural','Sugar Mills','Tourism','Transport','Tribal-SC/ST','Urban Cooperative Bank (UCB)','Women Welfare'
        data: [<?php
            echo implode(', ',$societies[2]);
            ?>],
        tooltip: {
            valueSuffix: ''
        }
